...
for (len=2; len<=max_length; len++) {
	num_divisors = split(length_table[len],divisors,";")
	
	divisors[len,0,0] = num_divisors
	divisors[len,0,1] = (10 ^ len) - 1
	for (i=1; i<=num_divisors; i++)
	{
		num_fields = split(divisors[i], divisor_info, ",")
...
/^[0-9]-/    { $1 = "11"; }
/^[0-9]{2}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{3}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{4}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{5}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{6}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{7}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{8}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{9}-/ { if (find_all_invalid_ids()) next; }
/^[0-9]{10}-/ { if (find_all_invalid_ids()) next; }
